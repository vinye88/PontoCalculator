...
class Jornada():
    ...
    def compensar_excesso(self, entrada, excesso):
        ...
        saida = almoco_end + (self.jornada - self.turno1) - self.gordura - self._d('00:11')
        if self._d(excesso) > self._d('00:11'):
            saida -= self._d(excesso) - self._d('00:11')
        return {
            'jornada':{
    ...
    def compensar_ausencia(self, entrada, ausencia):
        ...
        saida += self._d('00:11') if self._d(ausencia) <= self._d('00:11') else self._d(ausencia) - self._d('00:11')
        #min 11m or excesso
        return {
            'jornada':{
                'entrada':self._p(self._d(entrada)),
